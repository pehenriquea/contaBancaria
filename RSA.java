...
public class Conta {
    ...
    public byte[] toByteArray(Conta c1) throws Exception {
        ...
        geradorChaves();
        c1.senha = criptografar(c1.senha);
    ...
    public void toFilefromClass() {
        ...
        for (int j = 0; j < i; j++) {
            ...
            try {
                createDescompressao(c1, this.versaoCompressao);
            } catch (Exception e) {
    ...
    public static void geradorChaves() {
        // Gerando os pares de chaves
        try {
            KeyPairGenerator generator = KeyPairGenerator.getInstance("RSA");
            generator.initialize(2048); // a chave gerada terá tamanho de 2048 bits
            KeyPair par = generator.generateKeyPair();
            // Extraindo as chaves públicas e privadas
            privateKey = par.getPrivate(); // Encriptografar
            PublicKey publicKey = par.getPublic(); // Descriptografar

            // Salvando as chaves no arquivo public.key
    ...
    public static PublicKey leitorPublicKeyArquivo() {
    ...
    public static String criptografar(String senha) {
        String msgEncrip = null;
        try {
            Cipher encryptCipher = Cipher.getInstance("RSA");
            PublicKey publicKey = leitorPublicKeyArquivo();
            encryptCipher.init(Cipher.ENCRYPT_MODE, publicKey);
            byte[] secretMessageBytes = senha.getBytes(StandardCharsets.UTF_8); // passando a senha para um vetor de
                                                                                // bytes, porque é o tipo que a função
                                                                                // doFinal recebe
            byte[] encryptedMessageBytes = encryptCipher.doFinal(secretMessageBytes); // Função doFinal faz a
                                                                                      // cirptografia da mensagem
            msgEncrip = Base64.getEncoder().encodeToString(encryptedMessageBytes);
        } catch (Exception e) {
            System.err.println(e.getLocalizedMessage());
        }
        return msgEncrip;
    ...
    public static String descriptografar(String senha) {
        String msgDesc = null;
        try {
            geradorChaves(); // chamado apenas para gerar a privateKey
            Cipher decryptCipher = Cipher.getInstance("RSA");
            decryptCipher.init(Cipher.DECRYPT_MODE, privateKey);
            byte[] senhaBytes = senha.getBytes(StandardCharsets.UTF_8);
            byte[] decryptedMessageBytes = decryptCipher.doFinal(senhaBytes);
            msgDesc = new String(decryptedMessageBytes, StandardCharsets.UTF_8); //passando pra string o vetor de bytes descriptografado
        } catch (Exception e) {
            System.err.println(e.getMessage());
        }
